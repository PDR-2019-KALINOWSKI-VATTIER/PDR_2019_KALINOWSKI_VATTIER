...
""" Ce code, tout comme les codes depth_camera2.py, depth_camera3.py
    depth_camera4.py ne fonctionne pas.

    Il permettrait de lire les informations relatives au QR code detecte
    et de publier dans un nouveau topic la distance separant la camera
    du QR code."""
...
class depth_detection:
    ...
    def callback(self, ros_data):
        ...
        image_msg = rospy.wait_for_message("/camera/depth/image_rect_raw/compressedDepth", CompressedImage)
        if VERBOSE:
            print "Got image!"
...
#        print type(image_msg)
        ...
        print depths_np_arr[y0][x0]
        
        qr_dist = depths_np_arr[y0][x0][0]
        if VERBOSE:
